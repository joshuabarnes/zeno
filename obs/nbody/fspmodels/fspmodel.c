/*
 * FSPMODEL.C: make N-body model of FSP.
 */
...
string defv[] = {		";Make N-body model of FSP",
    "fsp=???",			";Input FSP for density profile",
    "out=",			";Output SnapShot with bodies",
    "grav=",		        ";Input FSP for gravity calculation",
    ...
    "VERSION=2.4",		";Josh Barnes  26 September 1997",
    NULL,
};

/* Prototypes for model construction and I/O.				    */

void fspmodel(void);			/* routine to construct model	    */
...

/*
 * FSPMODEL: construct realization from FSP data.
 */
...
void fspmodel(void)
{
	...
	r = r_mass_fsp(fsp, xrandom(0.0, mcut * fsp->mtot));
	vmax2 = -2 * rsqr(vcut) * phi_fsp(gfsp, r);
	if (vfac > 1.0)
	    vmax2 = vmax2 / rsqr(vfac);
	sigr = rsqrt(sig2_fsp(fsp, gfsp, beta_a, sig2, r));
...
void setauxvar(bodyptr btab, int nbody)
{
	...
	for (bp = btab; bp < NthBody(btab, nbody); bp = NextBody(bp))
	    Aux(bp) = mass_fsp(gfsp, absv(Pos(bp)));
	...
	for (bp = btab; bp < NthBody(btab, nbody); bp = NextBody(bp)) {
	    ...
	    r = 0.5 * (r0 + r1);
	    while ((r1 - r0) > TOL * r) {
		if (rsqrt(2 * (etot - phi_fsp(gfsp, r))) > jtot/r)
		    r1 = r;
		else
		    r0 = r;
		r = 0.5 * (r0 + r1);
