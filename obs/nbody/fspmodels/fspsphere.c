/*
 * FSPSPHERE.C: make SPH realization of FSP.
 */
...
string defv[] = {               ";Make SPH gas sphere from FSP",
    ...
    "VERSION=1.0",              ";Josh Barnes  26 January 1999",
    NULL,
};

/* Prototypes for model construction and I/O.                               */

void fspsphere(void);                   /* construct spherical SPH model    */
void readfsp(void);                     /* read input profile(s)	    */
...

/*
 * FSPSPHERE: construct realization from FSP data.
 */
...
void fspsphere(void)
{
    ...
    if (sig2tab == NULL)
        sig2tab = calc_sig2_fsp(fsp, gfsp, 0.0);
    if (btab == NULL)
	...
	CLRV(Vel(bp));
	Rho(bp) = rho_fsp(fsp, r);
	sig2 = sig2_fsp(fsp, gfsp, 0.0, sig2tab, r);
