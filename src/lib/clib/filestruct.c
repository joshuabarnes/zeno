...
{
  va_list ap;
  int dims[MaxVecDim + 1], ndim = 0, *mask;
		  ...
		  int *dims, void *data, int *mask)
{
  ...
  ostate = findstate(ostr);			// find corresponding state
  oitem = makeitem(type, tag, dims, data);	// make item representing data
...
int *get_dimensions(stream istr, string tag)
{
  ...
  if (indx == -1)
    error("%s.%s: %s not found\n", getprog(), iofunc, tag);
  end_iofunc();
  return (istate->buf[indx]->dims == NULL ? NULL :
	  copyistr(istate->buf[indx]->dims));	// return copy of item dims
}
 
...

//  finditem: scan current context for named item;
