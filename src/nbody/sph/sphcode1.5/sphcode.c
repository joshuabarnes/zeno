 ...
 * Copyright (c) 2013 by Joshua E. Barnes, Honolulu, Hawai'i.
 */
...
string defv[] = {		";SPH/N-body simulation code.",
...
#endif
#if defined(STARFORM)
  "csfr=0.0,0.0,0.0",		";Star formation law constants",
  "nsfr=1.0,1.0,1.0",		";Star formation density indicies",
  "msfr=0.0,0.0,0.0",		";Star formation shock indicies",
  ...
  "trace=",			";Output file name for trace frames",
  "log=",			";Output file name for simulation log",
  "VERSION=1.5",		";Joshua Barnes  14 May 2013",
  NULL,
};
...
local void newrun(void)
{
  ...
  sscanf(getparam("csfr"), "%f,%f,%f", &csfr[0], &csfr[1], &csfr[2]);
  sscanf(getparam("nsfr"), "%f,%f,%f", &nsfr[0], &nsfr[1], &nsfr[2]);
  sscanf(getparam("msfr"), "%f,%f,%f", &msfr[0], &msfr[1], &msfr[2]);
#if defined(MASSLOSS)
...
#endif
#if defined(STARFORM)
  if (getparamstat("csfr") & ARGPARAM)
    sscanf(getparam("csfr"), "%f,%f,%f", &csfr[0], &csfr[1], &csfr[2]);
  if (getparamstat("nsfr") & ARGPARAM)
    sscanf(getparam("nsfr"), "%f,%f,%f", &nsfr[0], &nsfr[1], &nsfr[2]);
  if (getparamstat("msfr") & ARGPARAM)
    sscanf(getparam("msfr"), "%f,%f,%f", &msfr[0], &msfr[1], &msfr[2]);
...
#endif
      rate =
	(csfr[0]>0 ? csfr[0]*rpow(rho,nsfr[0]-1)*rpow(udot,msfr[0]) : 0) +
	(csfr[1]>0 ? csfr[1]*rpow(rho,nsfr[1]-1)*rpow(udot,msfr[1]) : 0) +
	(csfr[2]>0 ? csfr[2]*rpow(rho,nsfr[2]-1)*rpow(udot,msfr[2]) : 0);
						// rate has units of 1/time
