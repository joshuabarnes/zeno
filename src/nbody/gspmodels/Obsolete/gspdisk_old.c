/*
 * GSPDISK.C: set up an exponential disk embedded in a gsp halo.
 */
...
string defv[] = {		";Make exponential disk in a gsp halo",
    "gsp=",			";Input gsp for halo mass profile",
...
gsprof *halo = NULL;			/* def. halo mass as fcn of radius  */
...
/*
 * READGSP: read halo GSP from input file.
 */
...
void readgsp(void)
{
    ...
    if (! strnull(getparam("gsp"))) {
	istr = stropen(getparam("gsp"), "r");
	get_history(istr);
	halo = get_gsprof(istr);
	strclose(istr);
    }
}

...
void setprof(void)
{
	...
	mhalo = (halo != NULL ? mass_gsp(halo, r) : 0.0);
